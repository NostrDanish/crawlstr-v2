...
import { discoverRelays } from '@/lib/relayDiscovery';
...
export function RelayManager() {
  ...
  const handleDiscover = async () => {
    ...
    try {
      const queryFn = async (relays: string[], filters: Array<Record<string, unknown>>) => {
        const group = nostr.group(relays);
        return await group.query(filters as never, { signal: AbortSignal.timeout(15000) }) as NostrEvent[];
      };
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-medium">Publish relays</h3>
        <p className="text-xs text-muted-foreground">
          Observations and heartbeats are pushed to every relay in this set. Built-ins can't be removed; your additions persist locally.
        </p>
      </div>
