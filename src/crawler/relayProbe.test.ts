...
import { probeRelay, probeRelays } from './relayProbe';
import { extractRelaysWithNip } from './relayDiscovery';
import type { NostrEvent } from '@nostrify/nostrify';

/**
 * SSRF guard on the NIP-11 probe path (audit finding #2).
 *
 * Relay URLs come from kind 30166 NIP-66 events — attacker-publishable
 * network data. Before the fix, a "relay" at ws://169.254.169.254 would be
 * fetched direct AND via the CORS proxy with no safety check. fetch is
 * mocked; the core assertion is that non-public targets cause ZERO requests.
 */
...
describe('relayProbe SSRF guard (audit finding #2)', () => {
  beforeEach(() => {
    vi.stubGlobal('fetch', vi.fn(async () =>
      new Response(JSON.stringify({ supported_nips: [1, 50] }), { status: 200 }),
    ));
  });
  afterEach(() => {
    vi.unstubAllGlobals();
  ...
  it('refuses a NIP-11 probe to 169.254.169.254 (cloud metadata)', async () => {
    const caps = await probeRelay('ws://169.254.169.254');
  ...
  it('probes a normal public relay and reports its capabilities', async () => {
    const caps = await probeRelay('wss://relay-public-example.com');
